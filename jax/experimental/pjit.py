...
def _prepare_axis_resources(axis_resources,
  ...
  all_auto = _check_if_all_auto(entries, arg_name)
  if not all_auto:
    entries = [
...
def _check_if_all_auto(axis_resources, name):
...
def parse_op_sharding(op_sharding: xc.OpSharding, mesh: pxla.Mesh) -> Sequence[ParsedPartitionSpec]:
  if op_sharding.type == xc.OpSharding.Type.TUPLE:
    out: List[ParsedPartitionSpec] = []
    for s in op_sharding.tuple_shardings:
      out.extend(parse_op_sharding(s, mesh))
    return out
...
def _get_ppspec_from_executable(executable, mesh) -> Tuple[Sequence[ParsedPartitionSpec], Sequence[ParsedPartitionSpec]]:
  ...
  for sharding in input_op_shardings:
    in_ppspec.extend(parse_op_sharding(sharding, mesh))
  out_ppspec = parse_op_sharding(output_op_sharding, mesh)
